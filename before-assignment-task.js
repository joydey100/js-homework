...
function paperRequirements(firstbookcopy, secondbookcopy, thirdbookcopy) {
  let pageForFirstBook = 100;
  let pageForSecondBook = 200;
  let pageForThirdBook = 300;

  let totalPage =
    firstbookcopy * pageForFirstBook +
    secondbookcopy * pageForSecondBook +
    thirdbookcopy * pageForThirdBook;
...
let myBestFriends = ["Sizan", "Sadit", "Tushar", "Alvi", "Rahul"];

function bestFriend(arr) {
  let largeName = [];
  let initialFriend = arr[0];
  for (let friend of arr) {
    if (
      friend.length > initialFriend.length &&
      largeName.indexOf(friend) == -1
    ) {
      largeName.push(friend);
    }
  }
  return largeName;
}
